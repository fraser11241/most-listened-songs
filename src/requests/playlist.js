...
export const addTopSongsFromArtistsToPlaylist = async (
	token,
	playlistId,
	artistIds,
	songsFromEachArtist = "5"
) => {
	const artistsTopTrackUris = await Promise.all(
		artistIds.map(async (id) => {
			return await getTopTracksFromArtist(token, id, songsFromEachArtist);
		})
	);
	const combinedUris = []
		.concat(...artistsTopTrackUris)
		.map(({ uri }) => uri);

	if (combinedUris) {
		addSongsToPlaylist(token, playlistId, combinedUris);
	}
};
